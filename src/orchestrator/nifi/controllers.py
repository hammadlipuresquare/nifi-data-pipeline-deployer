...
class ControllerServiceManager(LoggerMixin):
    ...
    def get_kafka_services(self, pg_id: str) -> List[ControllerServiceInfo]:
        ...
        for service in services:
            ...
            # Check if this is a Kafka-related service
            kafka_keywords = ["kafka", "confluent"]
            is_kafka_service = any(
                keyword in service_type.lower() or keyword in service_name.lower()
    ...
    def configure_kafka_services(self, pg_id: str, tenant_id: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        # Log configuration values (masked)
        self.logger.info("Configuration values being used:")
        masked_config = mask_sensitive_data(essential_config)
        ...
        try:
            ...
            if response:
                ...
                for service in controller_services:
                    ...
                    self.logger.info(f"  Controller Service: {service_info.get('name', 'Unknown')} -> {service_info.get('state', 'Unknown')}")
